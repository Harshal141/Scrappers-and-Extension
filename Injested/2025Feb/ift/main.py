...
def main():
    ...
    website= "https://ift25.mapyourshow.com/8_0/explore/exhibitor-gallery.cfm?featured=false"
        ...
        if spon_ul_list:
            for li in spon_ul_list.find_all('li'):
                ...
                if h3_tag and h3_tag.find('a'):
                    ...
                    full_domain = f"https://ift25.mapyourshow.com/{domain}"
...
def remove_duplicates_from_json(file_name='companies.json'):
    """
    Removes duplicate entries from the JSON file.
    
    :param file_name: The name of the JSON file to clean.
    """
    ...
    # Remove duplicates
    unique_data = list({(item['name'], item['website']): item for item in data}.values())
    
    # Save the cleaned data
    with open(file_name, 'w') as json_file:
        json.dump(unique_data, json_file, indent=4)
        logging.info('Duplicates removed and data saved to companies.json')
