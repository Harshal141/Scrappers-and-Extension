chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.action === "navigate_and_scrape") {
    (async () => {
      const { productList, startIndex, endIndex, instances, batchSize } =
        message;
      ...
      async function processProduct(product) {
        ...
        try {
          ...
          return {
            name: product.name,
            domain_links: Array.isArray(httpsTab.result)
              ? httpsTab.result
              : [httpsTab.result],
          };
        } catch (httpsError) {
          console.error(`Scrape failed for ${product.url}`, httpsError);
      ...
      const processInBatches = async (products) => {
        for (let i = 0; i < products.length; i += maxConcurrentTabs) {
          ...
          console.log(
            "Waiting for 5 seconds before processing the next batch..."
          );
          ...
          chrome.tabs.sendMessage(tabId, { action: "scrape" }, (response) => {
            ...
            if (chrome.runtime.lastError || !response) {
              reject({
                id: tab.id,
                error: new Error(
                  `Scraping failed for tab ID ${tab.id}: ${
